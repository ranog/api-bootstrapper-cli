...
def test_should_create_vscode_directory(tmp_path: Path):
    ...
    writer.write_python_interpreter(tmp_path, python_path)
...
def test_should_write_python_interpreter_setting(tmp_path: Path):
    ...
    settings_path = writer.write_python_interpreter(tmp_path, python_path)
    
    assert json.loads(settings_path.read_text())["python.defaultInterpreterPath"] == str(python_path)
...
def test_should_merge_with_existing_settings(tmp_path: Path):
    ...
    settings_file.write_text(
        json.dumps({"editor.fontSize": 14, "editor.tabSize": 4})
    )
    writer = VSCodeWriter()
    python_path = Path("/usr/bin/python3")
