...
@patch("api_bootstrapper_cli.commands.install_skills.install_codex_skills")
def test_should_install_skills_with_default_target(
...
@patch("api_bootstrapper_cli.commands.install_skills.install_codex_skills")
def test_should_install_skills_with_custom_target(
    mock_install: MagicMock, tmp_path: Path
):
    target_path = tmp_path / "custom-skills"
    mock_install.return_value = SkillsInstallResult(
        distribution=SkillsDistribution.codex,
        source_root=tmp_path / "source",
    ...
    result = runner.invoke(
        app,
        ["install-skills", "--target", str(target_path), "--no-overwrite"],
    )
...
@patch("api_bootstrapper_cli.commands.install_skills.install_codex_skills")
def test_should_show_skipped_skills_when_not_overwriting(
    mock_install: MagicMock, tmp_path: Path
):
    mock_install.return_value = SkillsInstallResult(
        distribution=SkillsDistribution.codex,
        ...
        installed_skills=("api-bootstrapper-init",),
        overwritten_skills=(),
        skipped_skills=("api-bootstrapper-add-docker",),
    )

    result = runner.invoke(app, ["install-skills", "--no-overwrite"])
    output = strip_ansi_codes(result.stdout)

    assert result.exit_code == 0
    assert "Skipped skills" in output
    assert "api-bootstrapper-add-docker" in output


@patch("api_bootstrapper_cli.commands.install_skills.install_codex_skills")
def test_should_return_error_when_installation_fails(mock_install: MagicMock):
    mock_install.side_effect = RuntimeError("missing embedded skills")
    ...
    assert "missing embedded skills" in output
